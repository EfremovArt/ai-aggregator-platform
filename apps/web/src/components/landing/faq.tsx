...
const ITEMS = [
  ...
  {
    q: 'Как считается стоимость? Есть ли подписка?',
    a: 'Подписок нет. Только pay-per-use: для текстовых моделей считаем по фактически использованным токенам (вход + выход), для изображений и видео — за единицу, для озвучки — за 1 000 знаков, для распознавания речи — за минуту. Полная оценка видна до отправки запроса.',
  },
  {
    q: 'Что такое токен?',
    a: '1 000 токенов ≈ 350–400 слов на русском или ≈ 750 слов на английском. При каждом запросе оплачивается и ваше сообщение, и вся история диалога (контекст), чтобы модель не теряла нить.',
  },
  {
